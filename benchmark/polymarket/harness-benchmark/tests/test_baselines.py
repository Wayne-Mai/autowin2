...
class BaselineTests(unittest.TestCase):
    ...
    def test_maker_single_trader_mirror_emits_limit_orders(self):
        trades, quotes = load_small_fixture()
        sim = ReplaySimulator(
            strategies=[MakerSingleTraderMirrorBaseline(wallets=["0xaaa"], max_notional=50)],
            fill_model=ConservativeFillModel(delay_seconds=60),
        )
        result = sim.run(trades, quotes)[0]

        self.assertGreaterEqual(result.metrics["orders"], 1)
        self.assertTrue(all(order.signal.execution_style == "maker" for order in result.orders))
        self.assertEqual(result.orders[0].signal.limit_price, trades[0].price)
