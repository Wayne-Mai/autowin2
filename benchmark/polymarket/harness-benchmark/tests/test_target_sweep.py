...
from polypaper.cli import (
    _collector_pinned_assets,
    _parse_float_list,
    _sweep_raw_top,
    _unique_sweep_assets,
)
...
class TargetSweepTests(unittest.TestCase):
    ...
    def test_collector_pinned_assets_combines_explicit_and_allowed_assets(self):
        args = SimpleNamespace(pin_assets="TOKEN-A,TOKEN-B", target_allowed_assets="TOKEN-B,TOKEN-C")

        self.assertEqual(_collector_pinned_assets(args), ["TOKEN-A", "TOKEN-B", "TOKEN-C"])
    ...
    def test_sweep_result_builds_runnable_target_strategy(self):
        ...
        strategy = target_strategy_from_sweep_result(
            SimpleNamespace(
                initial_cash=1000,
                portfolio_target_roi=0.10,
                stop_loss_pct=0.15,
    ...
    def test_sweep_raw_top_pulls_deep_pool_for_asset_diversity(self):
        self.assertEqual(_sweep_raw_top(SimpleNamespace(sweep_strategies=5)), 1000)
