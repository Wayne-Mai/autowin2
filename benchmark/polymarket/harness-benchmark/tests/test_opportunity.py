...
def paired_snapshot(asset, outcome_index, outcome, bid, ask):
    return MarketSnapshot(
        asset=asset,
        condition_id="0xpair",
        timestamp=1000 + outcome_index,
        book=OrderBook(
            asset=asset,
            timestamp=1000 + outcome_index,
            bids=(BookLevel(price=bid, size=1000),),
...
class OpportunityTests(unittest.TestCase):
    ...
    def test_paired_outcome_scan_finds_settlement_edge(self):
        opportunities = scan_paired_outcome_opportunities(
            [
                paired_snapshot("YES", 0, "Yes", bid=0.44, ask=0.45),
                paired_snapshot("NO", 1, "No", bid=0.44, ask=0.45),
            ],
            min_settlement_roi=0.10,
        )

        self.assertEqual(len(opportunities), 1)
        opportunity = opportunities[0]
        self.assertTrue(opportunity.viable)
        self.assertAlmostEqual(opportunity.cost_per_set, 0.90)
        self.assertAlmostEqual(opportunity.settlement_roi, 1.0 / 0.90 - 1.0)
        self.assertLess(opportunity.mark_roi, 0.0)

    def test_paired_outcome_scan_rejects_expensive_pair(self):
        opportunities = scan_paired_outcome_opportunities(
            [
                paired_snapshot("YES", 0, "Yes", bid=0.49, ask=0.50),
                paired_snapshot("NO", 1, "No", bid=0.49, ask=0.51),
            ],
            min_settlement_roi=0.10,
        )

        self.assertFalse(opportunities[0].viable)
        self.assertEqual(opportunities[0].reason, "settlement_roi_below_threshold")
    ...
    def test_target_opportunity_preserves_history_change(self):
        snapshot = replace(snapshot_with_book(0.49, [(0.50, 1000)]), history_change_pct=0.08)
        ...
        self.assertAlmostEqual(opportunity.history_change_pct, 0.08)
    ...
    def test_adaptive_target_opportunity_searches_between_min_failure_and_fee_capped_max(self):
        opportunity = score_adaptive_target_opportunity(
            snapshot_with_book(0.664, [(0.665, 1000)]),
            initial_cash=100,
            current_cash=100,
            max_target_notional=100,
            min_target_notional=5,
            portfolio_target_roi=0.10,
            take_profit_pct=0.10,
            ...
            min_book_imbalance=-1.0,
            max_entry_mark_to_bid_loss_pct=0.03,
            max_required_exit_distance_pct=0.15,
            required_exit_distance_weight=10.0,
            min_score=-1.0,
            fee_model=PolymarketFeeModel(fee_rate=0.03, exponent=1.0),
        )

        self.assertTrue(opportunity.viable)
        self.assertGreater(opportunity.target_notional, 99.0)
        self.assertLessEqual(opportunity.target_notional + opportunity.estimated_entry_fee, 100.0 + 1e-6)
